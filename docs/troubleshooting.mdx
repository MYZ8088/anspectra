...
description: Solutions for common issues when setting up and running AnswerLoom.
---
...
**Fix:** Open the provider connection again, complete the full sign-in flow (including any 2FA or confirmation steps), and wait for the checkmark to appear in the AnswerLoom UI before closing the browser window.
...
Note: these control sign-in to the AnswerLoom web app itself. They are separate from the Google / Gemini / AI Overview provider connections used for monitoring.
...
docker logs answerloom-migrate
...
3. Check `ANSWERLOOM_VPS_IP` and `AGENT_AUTH_UPLOAD_TOKEN` match your VPS `.env`
...
5. Check agent logs on the VPS: `docker logs answerloom-agent-worker`
