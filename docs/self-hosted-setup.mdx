...
description: Deploy the AnswerLoom app on your own VPS with nginx, SSL, and provider auth upload: step by step with copy-paste commands.
---
...
If you're on Ubuntu 22.04 / 24.04, copy your AnswerLoom checkout to the server first. The setup script then handles dependencies, `.env` configuration, Docker images, nginx, SSL, and firewall.

```bash
# On your VPS, from the AnswerLoom checkout:
...
When the script prompts for `Your domain for the app`, enter the hostname you want to serve AnswerLoom on, such as `app.yourdomain.com`. The script configures nginx and requests the TLS certificate for that exact domain.

Or configure your own repository URL and clone first:

```bash
git clone "$ANSWERLOOM_REPOSITORY_URL" /tmp/answerloom
bash /tmp/answerloom/scripts/setup-vps.sh
```
...
This guide deploys AnswerLoom on a fresh Ubuntu 22.04 / 24.04 VPS. At the end you'll have:

- AnswerLoom running behind nginx with HTTPS
...
- AnswerLoom collects responses from real provider UIs, not official model APIs
...
```bash
git clone "$ANSWERLOOM_REPOSITORY_URL" /home/$USER/answerloom
cd /home/$USER/answerloom
cp .env.example .env
```
...
- AnswerLoom collects from authenticated web UIs, so network reputation matters
...
```bash
sudo nano /etc/nginx/sites-available/answerloom
```
...
```bash
sudo ln -s /etc/nginx/sites-available/answerloom /etc/nginx/sites-enabled/
...
Before you run prompts from the VPS, make sure `THORDATA_PROXY_API_URL` in `/home/$USER/answerloom/.env` points to your real residential proxy endpoint. If you deployed with a placeholder or later changed the proxy configuration, update that value and rerun:
...
```bash
docker logs answerloom-web --tail 50
```
...
**Providers disconnected after reboot:** sessions survive restarts (they're in the volume at `/opt/answerloom/storage`). If they expire, re-run `pnpm upload:vps` from your local machine.
