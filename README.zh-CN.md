...
执行计划与提示词覆盖范围相互独立。用户可设置 1–30 次总执行次数；超过一次时，再选择每日、每周或每月周期。第一轮立即进入队列，后续轮次保存在同一个检测 series 中，因此报告可以直接计算回答稳定性，而不会把结果拆成互不相关的检测。
...
工作区 Owner 在 Settings 中自行填写兼容接口的 API Base URL、模型 ID 和 API Key。该模型只负责分析已经采集到的 Web 回答。Full Matrix 不会把 54 个回答放进同一个模型上下文：每个回答分别执行一次受 Schema 约束的分析，再由代码确定性聚合报告。
...
- 采集状态与分析状态分开保存。
- 结构化分析使用严格 JSON Schema、响应格式回退、平衡 JSON 提取、本地修复、Zod 校验和一次定向修复。
