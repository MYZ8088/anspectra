...
import { SourcesIntelligencePanel } from "@anspectra/ui";
import { useMemo } from "react";

export function SourcesMiniPreview(): React.JSX.Element {
	const metrics = useMemo<SourcePanelMetrics>(() => {
		const totalUrls = PREVIEW_SOURCE_GROUPS.reduce(
			(sum, row) => sum + row.urls,
			0,
		);
		const avgCitationsPerUrl =
			totalUrls > 0 ? (PREVIEW_TOTAL_CITATIONS / totalUrls).toFixed(1) : "0.0";
		return {
