...
describe("persistent provider profile ownership", () => {
	it("finds the primary Camoufox process and ignores content processes", () => {
		const profileDir = "/tmp/anspectra/runtime/hunyuan/profile";
		const processList = [
	...
	it("does not match a different provider profile", () => {
		const processList =
			"401 /Applications/Camoufox.app/Contents/MacOS/camoufox -profile /tmp/anspectra/runtime/doubao/profile";

		expect(
			findProfileOwnerPidInProcessList(
				processList,
				"/tmp/anspectra/runtime/hunyuan/profile",
			),
		).toBeNull();
	});
});
